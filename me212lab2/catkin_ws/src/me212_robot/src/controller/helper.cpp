...
void RobotPose::update(float dPhiL, float dPhiR) {
    ...
    dX = r/2*cos(Th)*(dPhiR+dPhiL);
    dY = r/2*sin(Th)*(dPhiR+dPhiL);
...
void PathPlanner::navigateTrajU(const RobotPose & robotPose) {
    ...
    float hemicircle_radius = 0.25;
    float buf = 0.18;
    // Straight line forward
    if (robotPose.pathDistance < 1.0) { 
        float robotVel = .02, K = 0;
        updateDesiredV(robotVel, K);
    } 
    // Hemicircle
    else if (robotPose.pathDistance < 1.0 + hemicircle_radius*PI + buf){
        float robotVel = .02, K = 1/hemicircle_radius;
        updateDesiredV(robotVel, K);
    }
    // Straight line back
    else if(robotPose.pathDistance < 2.0 + hemicircle_radius*PI + buf){
        float robotVel = .02, K = 0;
        updateDesiredV(robotVel, K);
    }
    // Stop at the end
...
void PathPlanner::updateDesiredV(float robotVel, float K) {
    ...
    // MODIFY CODE BELOW TO SET THE CORRECT VALUES
    desiredWV_R = robotVel/r*(1+K*b);
    desiredWV_L = robotVel/r*(1-K*b);
}


void PathPlanner::navigateTraj8(const RobotPose & robotPose) {
    // Use a series of IF statements to break up the path into steps.
    // Check whether you are ready to move forward into the next stage of the path
    
    // an example for the first straight line has been provided:
    float hemicircle_radius = 0.25;
    float K = 1/hemicircle_radius;
    float robotVel = .02;
    float buf = 0.25;
    // Straight line forward
    if (robotPose.pathDistance < 2*hemicircle_radius*PI+buf) { 
        updateDesiredV(robotVel, K);
    } 
    // Hemicircle
    else if (robotPose.pathDistance < 4*hemicircle_radius*PI+2*buf){
        updateDesiredV(robotVel, -K);
    }
    // Stop at the end
    else {
